...
function doCheckin() {
  ...
  if (!cookie && !auth) {
    $notification.post(CONFIG.NAME, "签到失败", "请先打开 ikuuu 网页抓取凭证");
    return $done();
  }
  ...
  $httpClient.post(
    ...
    (err, resp, data) => {
      ...
      fetchAccountInfo(headers, (info) => {
        ...
        if (/已签到|已经签到|already/.test((msg || "").toLowerCase())) {
          $notification.post(CONFIG.NAME, "今日已签到", detail);
          return $done();
        }

        if (j && (j.ret === 1 || j.success === true || /成功|success|获得/.test((msg || "").toLowerCase()))) {
